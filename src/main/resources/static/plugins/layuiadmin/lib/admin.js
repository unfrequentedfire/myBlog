...
;layui.define("view", function (e) {
    ...
    /*a("#LAY_app_tabsheader li").mouseover(function () {
        var tabId = a(this).attr("lay-id");
        alert(tabId);
    })*/

    //取消右键  rightmenu属性开始是隐藏的 ，当右击的时候显示，左击的时候隐藏
    a('#LAY_app_tabsheader li').on('contextmenu','li', function () {
        return false;
    });
    //tab点击右键
    a('#LAY_app_tabsheader').on('contextmenu','li', function (e) {
        var popupmenu = a(".layui-anim-fadein");
        //判断右侧菜单的位置
        l = (a(document).width() - e.clientX) < popupmenu.width() ? (e.clientX - popupmenu.width()) : e.clientX;
        t = (a(document).height() - e.clientY) < popupmenu.height() ? (e.clientY - popupmenu.height()) : e.clientY;
        popupmenu.addClass("layui-show");
        popupmenu.css({
            left: l,
            top: t,
        }).show(); //进行绝对定位
        popupmenu.css("min-width","auto");
        popupmenu.css("position","fixed");
        popupmenu.css("right","auto");
        return false;
    });
});