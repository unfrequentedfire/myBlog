...
layui.define(['laytpl', 'laypage'], function (exports) {
    ...
    var col12_tpl = '<div class="layui-row simple">' +
        ...
        '           {{# } }}' +
        '           <span class="layui-mr15"><i class="fa fa-calendar"></i> 时间：<span class="detail-date">{{ nbv5front.wholeCnDate(item.post) }}</span><span class="simple-date">{{ nbv5front.simpleDate(item.post) }}</span></span>' +
        '       </p>' +
        '   </div>' +
        '</div>' +
        '       {{# if(item.cover != null && item.cover !=""){ }}' +
